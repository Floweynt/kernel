...
namespace drivers::ps2
{
    namespace
    {
        ...
        constexpr std::array<std::uint8_t, TRANSLATION_TABLE_SIZE> us_qwerty_translation = {
            ...
            ']', 0x0D, 0x11, 'A', 'S', 'D', 'F', 'G', 'H', 'J', 'K', 'L', ';', '\'', '`', 0x0E, '\\', 'Z', 'X', 'C', 'V', 'B', 'N', 'M', ',', '.',
            '/', 0x0F, '*', 0x13, ' ', 0x02,
        ...
        // std::uint8_t decode_keyboard_scancode(std::uint8_t scancode, std::span<uint8_t, TRANSLATION_TABLE_SIZE> &translation_table, const
        // std::uint8_t flag_pressed)
        {
