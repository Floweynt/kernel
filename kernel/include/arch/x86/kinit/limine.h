...
#  define LIMINE_PTR(TYPE) std::uint64_t
#else
...
struct limine_file {
    ...
    std::uint32_t unused;
    std::uint32_t tftp_ip;
    std::uint32_t tftp_port;
    std::uint32_t partition_index;
    std::uint32_t mbr_disk_id;
...
struct limine_framebuffer_response {
    std::uint64_t revision;
    std::uint64_t framebuffer_count;
...
#define LIMINE_TERMINAL_CTX_SIZE ((std::uint64_t)(-1))
#define LIMINE_TERMINAL_CTX_SAVE ((std::uint64_t)(-2))
#define LIMINE_TERMINAL_CTX_RESTORE ((std::uint64_t)(-3))
#define LIMINE_TERMINAL_FULL_REFRESH ((std::uint64_t)(-4))
