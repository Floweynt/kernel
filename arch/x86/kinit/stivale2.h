...
#define _stivale2_split64(NAME)                                                                                             \
    union {                                                                                                                 \
        std::uint32_t NAME;                                                                                                      \
        std::uint32_t NAME##_lo;                                                                                                 \
    };                                                                                                                      \
    std::uint32_t NAME##_hi

#else

#define _stivale2_split64(NAME) std::uint64_t NAME
...
#define STIVALE2_TERM_CTX_SIZE ((std::uint64_t)(-1))
#define STIVALE2_TERM_CTX_SAVE ((std::uint64_t)(-2))
#define STIVALE2_TERM_CTX_RESTORE ((std::uint64_t)(-3))
#define STIVALE2_TERM_FULL_REFRESH ((std::uint64_t)(-4))
...
#define STIVALE2_PMR_EXECUTABLE ((std::uint64_t)1 << 0)
#define STIVALE2_PMR_WRITABLE ((std::uint64_t)1 << 1)
#define STIVALE2_PMR_READABLE ((std::uint64_t)1 << 2)
...
{
    struct stivale2_tag tag;
    std::uint64_t physical_base_address;
    std::uint64_t virtual_base_address;
};
...
struct stivale2_struct_tag_smbios
{
    struct stivale2_tag tag;
    std::uint64_t flags;
    std::uint64_t smbios_entry_32;
    std::uint64_t smbios_entry_64;
};
...
struct stivale2_smp_info
{
    ...
    std::uint64_t target_stack;
    std::uint64_t goto_address;
