...
void term_context_reinit(struct term_context* ctx)
{
    ...
    ctx->current_primary = (std::size_t)-1;
...
static void sgr(struct term_context* ctx)
{
    ...
    for (; i < ctx->esc_values_i; i++)
    {
        ...
        {
            ctx->bold = true;
            if (ctx->current_primary != (std::size_t)-1)
            {
                if (!ctx->reverse_video)
                {
        ...
        else if (ctx->esc_values[i] == 38 || ctx->esc_values[i] == 48)
        {
            ...
            case 5: { // 256 colors
                ...
                std::uint32_t col = ctx->esc_values[i + 1];
                ...
                {
                    std::uint32_t rgb_value = col256[col - 16];
                    (fg ? ctx->set_text_fg_rgb : ctx->set_text_bg_rgb)(ctx, rgb_value);
                }
...
static void dec_private_parse(struct term_context* ctx, std::uint8_t c)
{
    ctx->dec_private = false;
    ...
    {
        ctx->callback(ctx, TERM_CB_DEC, ctx->esc_values_i, (std::uintptr_t)ctx->esc_values, c);
    }
}
...
static void linux_private_parse(struct term_context* ctx)
{
    ...
    if (ctx->callback != nullptr)
    {
        ctx->callback(ctx, TERM_CB_LINUX, ctx->esc_values_i, (std::uintptr_t)ctx->esc_values, 0);
    }
}

static void mode_toggle(struct term_context* ctx, std::uint8_t c)
{
    if (ctx->esc_values_i == 0)
    {
        return;
    ...
    if (ctx->callback != nullptr)
    {
        ctx->callback(ctx, TERM_CB_MODE, ctx->esc_values_i, (std::uintptr_t)ctx->esc_values, c);
    }
}

static void control_sequence_parse(struct term_context* ctx, std::uint8_t c)
{
    ...
    for (std::size_t i = ctx->esc_values_i; i < TERM_MAX_ESC_VALUES; i++)
    {
    ...
    case 'A': {
        if (ctx->esc_values[0] > y)
            ctx->esc_values[0] = y;
        std::size_t orig_y = y;
        std::size_t dest_y = y - ctx->esc_values[0];
    ...
    case 'J':
        ...
        {
        case 0: {
            std::size_t rows_remaining = ctx->rows - (y + 1);
            std::size_t cols_diff = ctx->cols - (x + 1);
            std::size_t to_clear = rows_remaining * ctx->cols + cols_diff;
        ...
        case 1: {
            ...
            bool b = false;
            for (std::size_t yc = 0; yc < ctx->rows; yc++)
            {
                for (std::size_t xc = 0; xc < ctx->cols; xc++)
                {
                    ctx->raw_putchar(ctx, ' ');
                    if (xc == x && yc == y)
                    {
    ...
    case '@':
        ...
        break;
    case 'P':
        for (std::size_t i = x + ctx->esc_values[0]; i < ctx->cols; i++)
            ctx->move_character(ctx, i - ctx->esc_values[0], y, i, y);
        ctx->set_cursor_pos(ctx, ctx->cols - ctx->esc_values[0], y);
        // FALLTHRU
    case 'X':
        for (std::size_t i = 0; i < ctx->esc_values[0]; i++)
            ctx->raw_putchar(ctx, ' ');
        ctx->set_cursor_pos(ctx, x, y);
        break;
    case 'm':
    ...
    case 'K':
        ...
        {
        case 0: {
            for (std::size_t i = x; i < ctx->cols; i++)
                ctx->raw_putchar(ctx, ' ');
            ctx->set_cursor_pos(ctx, x, y);
            break;
        }
        ...
        case 2: {
            ctx->set_cursor_pos(ctx, 0, y);
            for (std::size_t i = 0; i < ctx->cols; i++)
                ctx->raw_putchar(ctx, ' ');
            ctx->set_cursor_pos(ctx, x, y);
            break;
        }
...
static void escape_parse(struct term_context* ctx, std::uint8_t c)
{
    ctx->escape_offset++;
    ...
    case '[':
    is_csi:
        for (std::size_t i = 0; i < TERM_MAX_ESC_VALUES; i++)
            ctx->esc_values[i] = 0;
        ctx->esc_values_i = 0;
        ctx->rrr = false;
...
static std::uint8_t dec_special_to_cp437(std::uint8_t c)
{
    switch (c)
    {
    case '`':
